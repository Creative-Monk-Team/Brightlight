...
let News = ({metaData}) => {
  ...
  let handleNewsClick = (news) => {
    localStorage.setItem("news_heading", news.news_heading);
    let newsSlug = news.news_heading
      .trim()
      .toLowerCase()
      .replace(/[^\w\s]/g, "")
      .replace(/\s+/g, "-");
    ...
    <>
      ...
      {firstBlog ? (
        <div className={styles.firstBlog}>
          ...
          <div>
            {firstBlog.image && <Image height={50} width={100} src={firstBlog.image} />}
          </div>
        </div>
      ) : null}
